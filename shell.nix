...
let
  ...
  pgtle14 = pkgs.callPackage ./nix/pg_tle {
    postgresql = xpgPkgs.postgresql_14;
  };
  pgtle15 = pkgs.callPackage ./nix/pg_tle {
    postgresql = xpgPkgs.postgresql_15;
  };
  pgtle16 = pkgs.callPackage ./nix/pg_tle {
    postgresql = xpgPkgs.postgresql_16;
  };
  pgtle17 = pkgs.callPackage ./nix/pg_tle {
  ...
  loadtestUtility = pkgs.writeShellScriptBin "supautils-loadtest-utility" ''
    ...
    EOF
  '';
  loadtestSelect = pkgs.writeShellScriptBin "supautils-loadtest-select" ''
    set -euo pipefail
